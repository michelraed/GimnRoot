...
void ProcessPulses(std::string fileName = "/home/gimn6/Documentos/Michel/DOUTORADO_CARACTERIZACAO/RootOutputs/PreProcessed/Caracterization/OV_QDC/time_120_t1_20_t2_20_te_15_ov_2_0_HW_OFF_qdcAquisicao_OV_04_02_2025_group_pre.root",
                   ...
                   std::string OutputFolder = "/home/gimn6/Documentos/Michel/DOUTORADO_CARACTERIZACAO/RootOutputs/PreProcessed/Caracterization/OV_QDC_FINAL")
{
    ...
    auto correctEnergyRVec = [&cPar](RVec<Float_t> *EnergyVec, int chipID)
    {
        ...
        for (int chan = 0; chan < 64; chan++)
        {
            ...
            Float_t EnergyWang = log(k1 / (k1 + b - energy)) * (1 / k2);
            Float_t EnergyNadig = c * s * log(1 / (1 - energy / s));
            Float_t EnergyPoli =pa * pow(energy, 2) + pb * energy + pc;
            Float_t EnergyPetSys =P0 * pow(P1, pow(energy, P2)) + P3 * energy + P0;
            ...
            correctedWang[auxChan]   = EnergyWang;
            correctedNadig[auxChan]  = EnergyNadig;
            correctedPoli[auxChan]   = EnergyPoli;
            correctedPetSys[auxChan] = EnergyPetSys;
    ...
    for (int entry = 0; entry < entries; entry++)
    {
       ...
       // std::cout<<"energy : "<<energia<<std::endl;
        ...
        Histogramas.FloodsRaw[chipIDp]->Fill(positionsWang["u"], positionsWang["v"]);

        if ((EnN > 425.0) && (EnN < 575.0))
            Histogramas.FloodsNadig_window[chipIDp]->Fill(positionsNadig["u"], positionsNadig["v"]);
        if ((EnW > 425.0) && (EnW < 575.0))
            Histogramas.FloodsWang_window[chipIDp]->Fill(positionsWang["u"], positionsWang["v"]);
        if ((EPol > 425.0) && (EPol < 575.0))
            Histogramas.FloodsPoli_window[chipIDp]->Fill(positionsPoli["u"], positionsPoli["v"]);
        if ((EPS > 425.0) && (EPS < 575.0))
            Histogramas.FloodsPetSys_window[chipIDp]->Fill(positionsPetSys["u"], positionsPetSys["v"]);


        //std::cout<<" energy "<<corrigidaWang<<" U : "<<positionsWang["u"]<<" V : "<<positionsWang["v"]<<std::endl;
        //std::cout<<" ewn" << EnW<<std::endl;
    ...
    for (auto &cID : Histogramas.chipIDs)
    {
        ...
        dir1->WriteObject(Histogramas.FloodsRaw[cID].get(), "Floods Raw");
        dir1->WriteObject(Histogramas.FloodsNadig_window[cID].get(), "Floods Nadig window");
        dir1->WriteObject(Histogramas.FloodsWang_window[cID].get(), "Floods Wang window");
        dir1->WriteObject(Histogramas.FloodsPoli_window[cID].get(), "Floods Poli window");
        dir1->WriteObject(Histogramas.FloodsPetSys_window[cID].get(), "Floods PetSys window");
