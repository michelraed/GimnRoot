...
void massiveCalibration(TString FolderTest="/home/gimn6/Documentos/Michel/DOUTORADO_CARACTERIZACAO/RootOutputs/PreProcessed/Caracterization/OV_QDC" ,
...
// /// Here you can configure your acquisition folder in order to process the data massively
    ...
    auto runCalibration= [](std::vector<TString>files){
        ProcessPulses(files[0].Data(), files[1].Data(),files[2].Data());
    };
    ...
    while ((file = (TSystemFile *)next()))
    {
        ...
        if (!file->IsDirectory() && TString(fileName).EndsWith("_group_pre.root", TString::kIgnoreCase))
        {
            ...
            std::vector<TString> files {fullPath,CalibrationFactorsFolder,OutFolder};
