...
void massiveTimeCoincidences(TString FolderTest="/home/gimn6/Documentos/Michel/DOUTORADO_CARACTERIZACAO/Aqd/OV/CoinQDC" ,
    ...
    TString OutFolder= "/home/gimn6/Documentos/Michel/DOUTORADO_CARACTERIZACAO/RootOutputs/PreProcessed/Caracterization/OV_QDC_COIN"
){
...
// /// Here you can configure your acquisition folder in order to process the data massively
    ...
    std::cout<<"IM HERE FELLA"<<std::endl;
    ...
    auto runCalibration= [](std::vector<TString>files){
        processCoincidencePreCal(files[0].Data(), files[1].Data());
    };
    ...
    while ((file = (TSystemFile *)next()))
    {
        ...
        if (!file->IsDirectory() && TString(fileName).EndsWith("_coinc.root", TString::kIgnoreCase))
        {
            ...
            std::vector<TString> files {fullPath,OutFolder};
